 ...
 *
 * The two editor-authored blocks (`stat`, `pillars`) get converters here too.
 * Their field shapes are declared below rather than imported from
 * `payload-types`: Payload types a Lexical body as the generic
 * `{ type: any; [k: string]: unknown }` tree, so there is nothing there to
 * import. These interfaces and `lib/payload/blocks/post-blocks.ts` are the two
 * halves of one contract, and the render test pins them together.
 */

/** `stat` block fields — see `lib/payload/blocks/post-blocks.ts`. */
...
/** `pillars` block fields — see `lib/payload/blocks/post-blocks.ts`. */
interface PillarsFields {
  blockType: 'pillars'
  chip?: string | null
...
type PostNodeTypes =
  | DefaultNodeTypes
  | SerializedBlockNode<PillarsFields>
  | SerializedBlockNode<StatFields>
...
/**
 * A figure lifted out of the prose it came from. The value carries its own
 * unit (`45%`, `3×`) — the editor wrote the claim, this only sets it.
 */
function StatAside({ fields }: { fields: StatFields }) {
  return (
    <aside className={s.stat}>
      <p className={s.statValue}>{fields.value}</p>
      <p className={s.statCaption}>{fields.caption}</p>
    </aside>
  )
}

/**
 * An ordered set the article refers to by number, so the ordinal is content
 * rather than decoration — it comes from array order and is rendered, not
 * generated by a list marker that a copy-paste would lose.
 */
function PillarsGrid({ fields }: { fields: PillarsFields }) {
  const chip = fields.chip?.trim()
  return (
    <ol className={s.pillars}>
      {fields.items.map((item, index) => {
        const description = item.description?.trim()
        return (
          // biome-ignore lint/suspicious/noArrayIndexKey: server-rendered from an immutable authored array — nothing reorders, inserts or removes an item, and the ordinal IS the identity the article refers to
          <li className={s.pillarCard} key={index}>
            <p className={s.pillarOrdinal}>{index + 1}</p>
            <p className={s.pillarTitle}>{item.title}</p>
            {description && <p className={s.pillarText}>{description}</p>}
            {chip && <p className={s.pillarChip}>{chip}</p>}
          </li>
        )
      })}
    </ol>
...
function makeConverters(
  ...
  locale: Locale,
  unoptimized: boolean,
  headingContent: Localized<typeof pl.postHeading> | undefined
): JSXConvertersFunction<PostNodeTypes> {
  ...
  return ({ defaultConverters }) => ({
    ...
    heading: ({ node, nodesToJSX }) => {
      ...
      if (Tag !== 'h2' || !headingContent) {
        return <Tag id={id}>{children}</Tag>
      }
      // Only `h2` gets the copy control, and only as a sibling — see
      // `HeadingAnchor` for why it must not sit inside the heading.
      return (
        <div className={s.headingRow}>
          <Tag id={id}>{children}</Tag>
          <HeadingAnchor content={headingContent} id={id} />
        </div>
      )
    },
    ...
    blocks: {
      stat: ({ node }) => <StatAside fields={node.fields} />,
      pillars: ({ node }) => <PillarsGrid fields={node.fields} />,
    },
  })
}
...
export function PostRichText({
  ...
  unoptimized: boolean
  /**
   * Copy for the `h2` copy-link control. Omitted on surfaces that render no
   * anchors — the case-study article, and the single-paragraph bodies `PostFaq`
   * renders per answer, which contain no headings at all.
   */
