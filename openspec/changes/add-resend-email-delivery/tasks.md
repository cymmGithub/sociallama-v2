...
- [ ] 1.1 `bun remove nodemailer @types/nodemailer && bun add resend`
- [ ] 1.2 `lib/env.ts`: replace the four `SMTP_*` entries with
      ...
      unchanged
- [ ] 1.3 `lib/utils/validation.ts`: `emailEnvSchema` requires
      `RESEND_API_KEY`, `EMAIL_FROM`, `CONTACT_INBOX`; update its doc comment
- [ ] 1.4 `lib/integrations/registry.ts`: email entry → name `Email (Resend)`,
      docsUrl `https://resend.com/docs`
- [ ] 1.5 `.env.example`: rewrite the email section for Resend (API key,
      `EMAIL_FROM` on the verified domain, both inboxes; drop the Google
      app-password walkthrough)

## 2. Delivery client and actions

- [ ] 2.1 Replace `lib/integrations/email/transport.ts` with `client.ts`:
      cached `getResend(): Resend | null`, `undefined`/`null` resolution
      convention and one-time warn preserved (design D2)
- [ ] 2.2 `lib/integrations/email/action.ts` (`sendContactEmail`): guard on
...
- [ ] 2.3 `lib/integrations/email/careers-action.ts`
...
- [ ] 2.4 Sweep for leftovers: no `nodemailer`/`SMTP_` reference remains in
      `lib/`, `app/`, `.env.example`, docs (`ARCHITECTURE.md`, `PROD-README.md`
      if they mention it)

## 3. Health probe and daily workflow

- [ ] 3.1 `lib/scripts/email-health.ts`: plain-fetch probe (zero imports) —
      ...
      (design D4)
- [ ] 3.2 Add `"email:health": "bun ./lib/scripts/email-health.ts"` to
      package.json scripts
- [ ] 3.3 `.github/workflows/email-health.yml`: daily cron at an off-hour
...
- [ ] 4.1 `bun run check` green (biome, tsc, tests, manifest — regenerate
      COMPONENTS.md if the manifest tracks the changed signatures)
- [ ] 4.2 Boot the dev server WITHOUT Resend env: `/kontakt` and
      `/zostan-lama` render, one warn logs, submissions return the failure
      state (fail-soft spec)
- [ ] 4.3 [BLOCKED: needs user secrets + verified domain] `bun run
      email:health` passes locally against `.env.local`
- [ ] 4.4 [BLOCKED: needs user secrets] Manual e2e: submit both forms on the
...
- [ ] 4.5 [BLOCKED: needs GitHub secret/variable] First `workflow_dispatch`
