...
5. **iRobot pillar removal is seed-first.** Delete the `#DLAKAŻDEGO` pillar from `seed-case-studies.ts`, re-run the idempotent seed against dev, then prod — keeps the seed the source of truth instead of a diverging DB write.
