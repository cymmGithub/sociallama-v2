...
The hero SHALL display the three-line headline ("STRATEGY / THAT WORKS / WITH SOCIAL LAMA") in the left column and the llama hero video in the right column, with the headline lines staggering in via GSAP synchronized to the llama's turn during the video's first loop, on a section background of exactly `#892f53`.

#### Scenario: Choreographed entrance
- **WHEN** the hero video begins its first playback
- **THEN** the headline lines animate in staggered, completing within the 3s first loop, and do not re-animate on subsequent loops

#### Scenario: Video not yet loaded
- **WHEN** the video has not started (slow network or reduced motion)
- **THEN** the poster is visible, the section background composites seamlessly with it, and the headline animates on a poster-visible fallback trigger
