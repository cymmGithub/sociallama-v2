# testimonial-rail — delta: depth-stack rail over six testimonials

Base spec: `testimonial-rail` as added by the `testimonial-pull-quote-rail` change (not yet archived to `openspec/specs/`). Note: that base text still describes pause-on-hover, which was already removed in code (9ed60a5); the MODIFIED autoplay requirement below is the corrected, current-intent version.
