...
- **Edge decontamination after matting** (`assets-src/hero-wardrobe/decontam.py`): the matte's semi-transparent edge keeps the source pixels' fur+gray-studio blend, which still reads as a faint halo on plum. Each edge pixel's RGB is replaced with the nearest fully-opaque pixel's color (alpha kept) and the alpha eroded 1px at 2K (invisible after the downscale) — the residual rim disappears.
