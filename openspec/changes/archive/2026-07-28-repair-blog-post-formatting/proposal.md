...
- **One CSS fix, added during implementation.** `.body` in `app/(frontend)/[slug]/post.module.css` is a flex column with a `gap`, but Payload's `<RichText>` wraps the body in its own `.payload-richtext` div — so the gap only ever applied between that wrapper and the in-article CTA, never between paragraphs, which carry no margin. Measured paragraph gap was 0px. The imported bodies hid it: their spacer paragraphs supplied the rhythm by accident, so removing those exposed it. `display: contents` on the wrapper lifts the blocks into the flex container. This contradicts the original "not touched: layout" claim below, and is why: without it the mechanical repair makes the prose worse, not better.

- **Not touched**: page width. Measured chars-per-line is already 75–83, above the readable ceiling, so widening `--post-measure` would make the prose worse. The separate composition question — the 1360px header stage stepping down to a 1008px article — is handled as its own mock-first CSS change.
