...
- [x] 0.1 **CLOSED WITHOUT SIGN-OFF (user decision, 2026-07-29).** No written consent was obtained from iRobot, Volvo or Pracuj.pl. The figures shipped to `main` anyway: iRobot's starting point (`1 168`), follower and likes growth (`+5 054`, `+57 911`) and annual YouTube subscriber growth, plus Pracuj.pl's AR-filter figures (`6,79 mln`, `4 885`). Mitigating context: `main` is pre-launch, not the live site, and everything except the iRobot baseline was already public in the case studies. The risk was raised three times before this decision and is recorded here so it is not mistaken for consent that was actually given.
...
- [x] 0.4 **CLOSED AS NOT-A-SITE-DEFECT (user decision, 2026-07-29).** Investigated in full and nothing was changed, because nothing on the site is wrong:
  ...
  - `seed-case-studies.ts` reproduces the client-facing deck faithfully, so **the site matches its source**. The contradiction is between two Social Lama documents; editing the site would only make it disagree with the deck the client was handed.
  - Suggestive but unprovable: the annual report's AR-filter slide (TikTok Effect House) reports `183 706` likes *for the filter alone* — within 0.2% of the `+184 000` booked as whole-profile engagement, while the neighbouring comment (`+4 249` vs `4 885`) and share (`+22 000` vs `27 607`) figures do not correspond that way. Consistent with one mis-copied cell in the annual report.
  - **Which figure is authoritative remains an owner decision about internal reporting, not an engineering task.** Step 03's own figures were verified directly against the Effect House panel and are unaffected.
## 1. Assets
