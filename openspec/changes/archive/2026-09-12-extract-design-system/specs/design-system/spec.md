...
Radius, elevation and motion-duration scales SHALL be defined as custom properties in a hand-authored `@theme` block in `lib/styles/css/tokens.css`, imported by `lib/styles/css/index.css`. Each token's value SHALL be one the shipped site already uses, chosen as the most-used member of its cluster, and token names SHALL describe role (for example `--radius-pill`, `--radius-card`, `--radius-chip`, `--shadow-card`, `--duration-fast`, `--duration-base`, `--duration-slow`), not the value.
