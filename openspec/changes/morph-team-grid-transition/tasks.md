...
- [x] 1.1 Check the Next 16 view-transitions guide for required config
...
- [x] 1.2 Wire a minimal, unpolished morph: `ViewTransition name` on one
      grid tile's image and on the slider's featured slot, hardcoded to one
      member — no wipe suppression, no crop tuning
- [x] 1.3 Test whether a scroll performed inside the navigation commit
...
- [x] 1.4 Verify the home ⇄ /o-nas round trip under the Activity cache:
...
- [x] 1.5 Judge the cover→contain crop behavior of the default morph;
      note whether `::view-transition-old/new` object-fit CSS is needed
- [x] 1.6 Confirm the custom `Link` / `Image` wrappers pass through what the
      wiring needs (or note the smallest passthrough change required)
- [x] 1.7 **Decision point**: record spike findings in design.md (Open
...
- [x] 2.1 Extract an async `ONasNews` child that awaits `getLatestPost()`
      and renders `NewsLama`, wrapped in `Suspense` with a news skeleton on
      the plum-deep band; make `ONasPage` sync (mirror `HomeNews`,
      home/page.tsx)
- [x] 2.2 Apply the same split to `/en/about-us`
- [x] 2.3 Verify with `lib/scripts/check-prerender` (or the house
...
- [x] 3.1 Derive the slider's initial `index` from `?lama=` at first client
...
- [x] 3.2 Add the pre-paint landing path to `ScrollReset`: on pathname
...
- [x] 3.3 Verify: client-nav from a tile paints frame one with the correct
...
- [x] 4.1 Land the config flag (if required) in `next.config.ts`
- [x] 4.2 Name each grid tile `team-<slug>` in `why-that-works`, and the
      featured slot (only — never peers) with the active member's slug in
      the team slider; apply on both locales
- [x] 4.3 Pin the fixed header with its own `ViewTransition` name
- [x] 4.4 Suppress the wipe-reveal entrance on morphing arrivals only
      (deep-linked client nav); direct visits keep the wipe, including its
      settled clip state
- [x] 4.5 Add the `prefers-reduced-motion` gate on the `::view-transition`
      pseudos, and the crop-tuning CSS if spike finding 1.5 called for it

## 5. Verification (gated on 1.7 go)

- [x] 5.1 Playwright pass on PL + EN: tile click morphs to the on-screen
...
- [x] 5.2 Regression sweep on previously-bitten areas: home ⇄ o-nas ⇄ home
      ...
      hash visit
- [x] 5.3 Confirm no-JS static shell parity (onas-team spec scenario) and
      run the deterministic a11y gate; `bun run check` + full test suite
      green

## 6. Documentation

- [x] 6.1 Update design.md Open Questions with spike answers and any
