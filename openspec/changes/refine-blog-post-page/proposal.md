...
The post template renders as a single centred column: meta, title, lead, a wide cover, a 70ch body, and an author card at the very bottom (`app/(frontend)/[slug]/page.tsx`, `post.module.css`). It reads as a document, not as a publication. Two things are missing that every editorial site the user benchmarked against provides: **orientation** (no table of contents, so a 9-minute post gives no map) and **brand presence** (the page uses none of the plum grain stage language that carries the rest of the site). Attribution itself stays where it is — one author card after the body (user decision 2026-07-26: an earlier revision also put the author above the table of contents, which just printed the same fields twice).

Design direction was settled from mocks reviewed with the user (2026-07-26); the chosen post page is `post-final.html` in the session mock set, with the two author states in `rail-states.html` (those states now render on the bottom card rather than in the rail).
...
- **Sticky left rail** carrying a **table of contents** with scroll-spy and share links. Desktop only; on mobile the TOC collapses above the body and the share links move below it. The rail carries no byline — the bottom card remains the page's single attribution surface.
...
- **`authors` gains an optional `role`** (e.g. "Specjalista SEO, SEOFLY"), shown on the author card. Posts with no author keep resolving to the Social Lama organization, which renders as the reversed lama mark on a plum disc labelled "Zespół redakcyjny" — a deliberate brand state, not a missing photo.
...
- `blog-authors`: authors gain an optional `role`, shown on the bottom card; the card is fixed as the post's single attribution surface, and the organization fallback gains an explicit presented role plus a reversed-mark treatment.
