...
- **Reorder the roster** on the homepage grid and the sliders to the client's new order: Anna, Kamil, Robert, Emilia, Paulina, Magda, Piotrek, Agnieszka, Kasia, Oliwia, Karolina, Wojtek, Aleksander, Przemek (14 members). The previous position-seniority ordering rule is replaced by this client-curated order; grid and slider now share it exactly.
...
- `onas-team`: roster membership grows from 12 to 14 (Wojtek, Aleksander in; nobody out); ordering requirement changes from position-seniority-with-curated-deviation to a single client-curated order shared by grid and slider; Robert's portrait asset is replaced; bio requirements extend to the two new members in both locales.
