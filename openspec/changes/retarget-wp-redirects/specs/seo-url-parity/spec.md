...
Redirect targets SHALL be dedicated v2 routes without URL fragments. Crawlers ignore fragments in redirect targets, so an anchor destination (e.g. `/#uslugi`) consolidates equity into the bare path instead of the intended section; a dedicated route is always the closer surviving equivalent. Specifically: `/oferta`, every `/oferta/{platform}` page, and `/500-zl-na-reklame` SHALL 301 to `/uslugi`; `/z-lama-warto` SHALL 301 to `/o-nas`.
...
- **WHEN** `/oferta/` or any `/oferta/{platform}/` URL (facebook, instagram, linkedin, tiktok, twitter, pinterest) is requested
