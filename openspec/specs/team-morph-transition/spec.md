# team-morph-transition Specification

## Purpose
The view-transition behavior between the homepage team grid and the `/o-nas`
team slider: shared-element morph on supported desktop browsers, crossfade
default, suppression of competing entrance animations, and reduced-motion,
mobile, and unsupported-browser fallbacks.
## Requirements
...
The morph applies to the grid presentation only: below the house desktop
breakpoint (`breakpoints.dt`, where the grid renders as a horizontal rail)
neither surface SHALL carry transition names, so no view transition
activates and the navigation behaves as the instant, correctly-sequenced
arrival (user decision, 2026-08-04).

#### Scenario: Morph runs on a supported browser

- **WHEN** a visitor on a view-transition-capable browser at desktop width
  clicks the homepage tile for a member
...
#### Scenario: Mobile rail never morphs

- **WHEN** a visitor at a viewport below the desktop breakpoint taps a team
  tile on the mobile rail
- **THEN** no view transition activates — the navigation completes as an
  instant, correctly-sequenced arrival and the slider keeps its wipe
  entrance
...
mobile viewports, or any state where the shared element cannot participate —
the navigation SHALL fall back to, at minimum, today's instant behavior with
the corrected arrival sequencing (right member, right scroll position at
first paint). A plain page crossfade without the element morph is an
accepted degraded mode; a broken or partial animation is not.
